...
// scratch - a temporary variable the same length as the block
// block - the block of the output to calculate
fn calculate_block_without_salt<M: Mac>(
    mac: &mut M,
    c: u32,
    idx: u32,
    scratch: &mut [u8],
 ...
 *
 */
pub fn pbkdf2_without_salt<M: Mac>(mac: &mut M, c: u32, output: &mut [u8]) {
    ...
    for chunk in output.chunks_mut(os) {
        ...
        if chunk.len() == os {
            calculate_block_without_salt(mac, c, idx, &mut scratch, chunk);
        } else {
            let mut tmp: Vec<u8> = repeat(0).take(os).collect();
            calculate_block_without_salt(mac, c, idx, &mut scratch[..], &mut tmp[..]);
            let chunk_len = chunk.len();
            copy_memory(&tmp[..chunk_len], chunk);
        }
    }
